...
function build_wpk_linux() {
  ...
  docker run -t --rm -v ${KEYDIR}:/etc/wazuh:Z -v ${DESTINATION}:/var/local/wazuh:Z \
      -v ${CHECKSUMDIR}:/var/local/checksum:Z \
      ${CONTAINER_NAME} ${BRANCH} ${JOBS} ${OUT_NAME} ${CHECKSUM} ${INSTALLATION_PATH} "" ${MANAGER_IP}
...
function help() {
  ...
  echo "    -m,   --manager_ip                          [Optional] Customize manager_ip on package generation"
  ...
  if [[ "$HAVE_TARGET" == true ]] && [[ "$HAVE_BRANCH" == true ]] && [[ "$HAVE_DESTINATION" == true ]] && [[ "$HAVE_KEYDIR" == true ]] && [[ "$HAVE_OUT_NAME" == true ]]; then
      if [[ "${TARGET}" == "windows" ]]; then
        ...
        local CONTAINER_NAME="${LINUX_BUILDER}"
          build_wpk_linux ${BRANCH} ${DESTINATION} ${KEYDIR} ${CONTAINER_NAME} ${JOBS} ${OUT_NAME} ${CHECKSUM} ${CHECKSUMDIR} ${INSTALLATION_PATH} ${MANAGER_IP} || clean ${LINUX_BUILDER_DOCKERFILE} 1
